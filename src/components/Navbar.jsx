...
const Navbar = () => {
  ...
  return (
    <div
      className={`fixed z-[100] w-full pl-10 pr-0 py-5 font-["Neue_Montreal"] flex justify-between items-center transition-all duration-300 ${
        isScrolled ? 'backdrop-blur-md bg-opacity-80 bg-transparent' : 'bg-transparent'
      }`}
      ref={navbarRef}
    >
      <div className="logo">
      ...
      <div className={`md:hidden cursor-pointer pr-8 ${isMobileMenuOpen ? 'z-[200]' : ''}`} onClick={toggleMobileMenu}>
      ...
      {/* Desktop Links */}
      <div className="hidden md:flex justify-end w-full h-[5vh] px-5">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            className="fixed inset-0 min-h-screen bg-black bg-opacity-95 z-[50] flex flex-col items-start justify-center pl-10 md:hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            {navLinks.map((link, index) => (
              <motion.a
                ...
                className="text-4xl font-bold my-4 text-white hover:text-gray-400 transition-colors duration-300 hover:underline"
